...
4. Otherwise derive `features/<ticket-id>-<short-kebab-summary>` when there is a ticket ID, or `features/<short-kebab-summary>` when there is not.
5. Preserve an obvious repo prefix such as `features/` only if the ticket ID remains visible.
